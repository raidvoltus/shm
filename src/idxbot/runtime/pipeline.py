...
class AutonomousPipeline:
    ...
    def run(
        ...
        *,
        ...
        for sym, pr in symbol_data.items():
            ...
            oi = create_order_intent(
                symbol=sym, timestamp=scheduled_at, intent=mapped["intent"],
    ...
    def _simple_feature_row(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        closes = []
        for r in rows:
            c = r.get("adjusted_close") or r.get("close") or 0
            closes.append(float(c))
        if len(closes) < 5:
            return {"mom_5": 0.0, "mom_20": 0.0, "vol_20": 0.0, "rsi_14": 50.0}
        mom5 = (closes[-1] / closes[-5] - 1.0) if closes[-5] else 0.0
        mom20 = (closes[-1] / closes[-20] - 1.0) if len(closes) >= 20 and closes[-20] else 0.0
        import statistics
        rets = [(closes[i] - closes[i-1]) / closes[i-1] for i in range(1, len(closes)) if closes[i-1]]
        vol20 = statistics.pstdev(rets[-20:]) if len(rets) >= 5 else 0.0
        ...
        rsi = 100 - (100 / (1 + rs))
        return {"mom_5": mom5, "mom_20": mom20, "vol_20": vol20, "rsi_14": rsi}

    def _momentum_horizons(self, rows: list[dict[str, Any]]) -> dict[str, float]:
        closes = [float(r.get("close") or r.get("adjusted_close") or 0) for r in rows]
        closes = [c for c in closes if c > 0]
        out = {"1D": 0.5, "5D": 0.5, "20D": 0.5}
        if len(closes) >= 2:
            out["1D"] = min(0.99, max(0.01, 0.5 + (closes[-1] / closes[-2] - 1) * 5))
        if len(closes) >= 6:
            out["5D"] = min(0.99, max(0.01, 0.5 + (closes[-1] / closes[-6] - 1) * 3))
        if len(closes) >= 21:
            out["20D"] = min(0.99, max(0.01, 0.5 + (closes[-1] / closes[-21] - 1) * 2))
        return out
    ...
    def _notify(
        ...
        intents: list[OrderIntent],
        stages: list[StageResult],
        portfolio_message: Optional[str] = None,
    ) -> bool:
        any_sent = False
        for oi in intents:
            try:
                ...
                msg = None
                if sd is not None:
                    msg = compose_with_fallback(sd, oi)
